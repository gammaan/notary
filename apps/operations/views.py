...
from operations.mixins import ManagerRequiredMixin, StaffListPrintMixin, StaffRequiredMixin, UUIDSlugMixin
...
class ClientListPrintView(StaffListPrintMixin, StaffRequiredMixin, ListView):
    model = Client
    template_name = "operations/print/clients_list.html"
    context_object_name = "clients"
...
class MatterListView(StaffRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        params.pop("page", None)
        ctx["filter_query"] = params.urlencode()
        ctx["clear_url"] = reverse("staff:matter_list")
        return ctx


class MatterListPrintView(StaffListPrintMixin, StaffRequiredMixin, ListView):
    model = Matter
    template_name = "operations/print/matters_list.html"
    context_object_name = "matters"
...
class TransactionListPrintView(StaffListPrintMixin, StaffRequiredMixin, ListView):
    model = Transaction
    template_name = "operations/print/transactions_list.html"
    context_object_name = "transactions"
...
class CalendarView(StaffRequiredMixin, TemplateView):
    ...
    def _day_payload(self, day: date, matters: list, focus_date: date | None) -> dict:
        is_expanded = focus_date == day
        preview_limit = len(matters) if is_expanded else self.PREVIEW_LIMIT
        preview = matters[:preview_limit]
        overflow = max(0, len(matters) - len(preview))
        return {
            "date": day,
            "matters": matters,
            "preview": preview,
    ...
    def get_context_data(self, **kwargs):
        ...
        if date_param:
            try:
                focus_date = date.fromisoformat(date_param)
                week_start = self._week_start(focus_date)
            except ValueError:
                focus_date = None
        ...
        for matter in matters:
            if matter.scheduled_at:
                matter_day = matter.scheduled_at.date()
                if matter_day in by_day:
                    by_day[matter_day].append(matter)

        week_days_data = [
            self._day_payload(day, by_day[day], focus_date) for day in week_days
        ]

        ctx["calendar_rows"] = [
            {"days": week_days_data[0:3], "variant": "work"},
            {"days": week_days_data[3:6], "variant": "work"},
            {"days": week_days_data[6:7], "variant": "off"},
        ]
        ...
        ctx["today"] = today
        ctx["focus_date"] = focus_date
        ctx["preview_limit"] = self.PREVIEW_LIMIT
        return ctx
...
class AuditLogListView(ManagerRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return filter_audit_logs(qs, self.request.GET)
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["staff_users"] = User.objects.filter(is_staff=True).order_by("first_name", "last_name", "email")
        ctx["active_entity"] = self.request.GET.get("entity", "")
        ctx["active_action"] = self.request.GET.get("action", "")
        ctx["active_user"] = self.request.GET.get("user", "")
        ctx["filters"] = self.request.GET
        params = self.request.GET.copy()
        params.pop("page", None)
        ctx["filter_query"] = params.urlencode()
        ctx["clear_url"] = reverse("staff:audit_log")
        return ctx


class AuditLogListPrintView(StaffListPrintMixin, ManagerRequiredMixin, ListView):
    model = AuditLog
    template_name = "operations/print/audit_list.html"
    context_object_name = "entries"
...
class AppointmentListView(StaffRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        params.pop("page", None)
        ctx["filter_query"] = params.urlencode()
        ctx["clear_url"] = reverse("staff:appointment_list")
        return ctx
...
class AppointmentListPrintView(StaffListPrintMixin, StaffRequiredMixin, ListView):
    model = AppointmentRequest
    template_name = "operations/print/appointments_list.html"
    context_object_name = "appointments"
