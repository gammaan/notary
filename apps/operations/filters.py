...
from operations.models import AppointmentRequest, AuditLog, Client, Document, Matter, Transaction
...
def filter_appointments(queryset, params):
    ...
    preferred_from = parse_date(params.get("preferred_from", "") or "")
    if preferred_from:
        queryset = queryset.filter(preferred_date__gte=preferred_from)

    preferred_to = parse_date(params.get("preferred_to", "") or "")
    if preferred_to:
        queryset = queryset.filter(preferred_date__lte=preferred_to)
...
def filter_audit_logs(queryset, params):
    ...
    if entity in AuditLog.EntityType.values:
        queryset = queryset.filter(entity_type=entity)
